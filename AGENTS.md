...
在进入 Plan 或 Code 模式前，必须遵守以下自动化约束（对应 `DEV.md` 中的 9 步走流程）：

1. **状态先行 (Step 1)**：所有非 trivial 任务启动或切换阶段时，必须更新 `docs/STATUS.md`。
2. **计划存档 (Step 2)**：详细计划必须存储在 `docs/plans/` 目录下。
3. **验证闭环 (Step 7)**：代码实现后必须生成测试报告并存入 `docs/reports/`。
4. **文档同步 (Step 8)**：任务结束前必须同步 `DOCS.md` 中的架构、知识库或变更记录。
5. **方法论回哺 (Step 9)**：任务彻底结束后，必须总结经验并建议更新本 `AGENTS.md`。
...
3. 偏好 **最小、可审阅的修改**：
   ...
   * 如需展示完整文件，应标明关键变更区域。
4. **引导验证 (Transition to Step 7)**：
   * 代码实现完成后，必须立即说明如何验证改动（测试、命令、人工检查步骤）。
   * 必须在下一条回复中通过 `docs/reports/` 记录验证证据。
...
1. **STATUS.md 维护 (Mandatory)**：
   * 必须在每轮非 trivial 会话中更新 `docs/STATUS.md`。
